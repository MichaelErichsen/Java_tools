...
public class Fonkod {
	...
	private String finalizeProcessing(String input) {
		...
		// sC_
		out = out.replaceFirst("([aeiouyæøå]+[^aeiouyæøå]*s[^aieouyæøå])\\b",
		"$1");

		// sC...s/V
		out = out
		.replaceFirst(
				"([aeiouyæøå]+[^aeiouyæøå]*s[^aieouyæøå])[^aeiosuyæøå]*([aeiosuyæøå]).*",
		"$1$2");

		// sV -> sr
		out = out.replaceFirst("([aeiouyæøå]+[^aeiouyæøå]*+s)[aieouyæøå].*",
		"$1r");

		// (C^s)s
		out = out.replaceFirst("([aeiouyæøå]+[^aeiouyæøå]*[^aieosuyæøå]s).*",
		"$1");

		// (C^s)(C^s)...s/V
		out = out
		.replaceFirst(
				"([aeiouyæøå]+[^aeiouyæøå]*[^aieosuyæøå]{2})[^aeiosuyæøå]*([aeiosuyæøå]).*",
		"$1$2");

		// (C^s)(C^s)_
		out = out.replaceFirst(
				"([aeiouyæøå]+[^aeiouyæøå]*[^aieosuyæøå]{2})\\b", "$1");

		// (C^s)...s/V
		out = out
		.replaceFirst(
				"([aeiouyæøå]+[^aeiouyæøå]*[^aieosuyæøå])[^aeiosuyæøå]*([aeiosuyæøå]).*",
		"$1$2");
	...
	private String performInitialStandardization(String nextToken) {
		...
		Pattern p = null;
		Matcher m = null;
		String[] patterns = { "vo", "vø", "val", "ver", "vie", "vet", "vah",
		"vri" };
	...
	private String processFirstVowelGroup(String input) {
		...
		if (m.group().length() < 2) {
			...
			if (input.charAt(st) == 'y')
				return input + " " + input.substring(0, st) + 'ø'
	...
	private String processSecondConsonantGroup(String input) throws Exception {
		...
		// Bn/ld -> Bn/l
		out = out.replaceFirst("([^aeiouyæøå]*[aeiouyæøå]+[^aeiouyæøå]*[ln])d",
		"$1");
		...
		// VdC -> VC
		out = out.replaceFirst("([^aeiouyæøå]*[aeiouyæøå]+)d([^aeiouyæøå])",
		"$1$2");
		...
		if (m.find()) {
			...
			// Bd -> Bt
			out = out.replaceFirst("([^aeiouyæøå]*[aeiouyæøå]+[^aeiouyæøå]*)d",
			"$1t");
		}
		...
		// Bb -> Bp
		out = out.replaceFirst("([^aeiouyæøå]*[aeiouyæøå]+[^aeiouyæøå]*)b",
		"$1p");

		// eg n/l ø/_/C/e -> ej n/l ø/_/C/e
		out = out
		.replaceFirst(
				"([^aeiouyæøå]*[aiouyæøå]+e)g([ln])([^aiouyæå]|\\b)",
		"$1j$2$3");

		// Vgø/_/C -> Vø/_/C
		out = out.replaceFirst("([^aeiouyæøå]*[aeiouyæøå]+)g([^aeiouyæå]|\\b)",
		"$1$2");
		...
		out = out.replaceFirst(
				"([^aeiouyæøå]*[eiuyæøå]*[ao]v)g([^aeiouyæå]|\\b)", "$1$2");
		...
		out = out
		.replaceFirst(
				"([^aeiouyæøå]*[aeiouyæøå]+[^aeiouyæøå]*[lr])g([^aiouyæå]|\\b)",
		"$1$2");

		// arC -> aC, ar -> a
		out = out.replaceFirst(
				"([^aeiouyæøå]*[eiouyæøå]*a)r([^aeiouyæøå]|\\b)", "$1$2");
		...
		if (m.find()) {
			...
			// Bg -> Bk
			out = out.replaceFirst("([^aeiouyæøå]*[aeiouyæøå]+[^aeiouyæøå]*)g",
			"$1k");
		}
		...
		out = out
		.replaceFirst(
				"([^aeiouyæøå]*[aeiouyæøå]+[^aeiouyæøå]*n)[gk]([^aeiouyæøå]|\\b)",
		"$1$2");

		// Bs d/t -> Bs
		out = out.replaceFirst("([^aeiouyæøå]*[aeiouyæøå]+[^aeiouyæøå]*s)[dt]",
		"$1");
		...
		// sch V/_ -> sj V/_
		out = out.replaceFirst(
				"([^aeiouyæøå]*[aeiouyæøå]+s)ch([aeiouyæøå]|\\b)", "$1j$2");

		// schC -> sC
		out = out.replaceFirst("([^aeiouyæøå]*[aeiouyæøå]+s)ch([^aeiouyæøå])",
		"$1$2");
	...
	private String processSecondVowelGroup(String input) throws Exception {
		...
		if (vgStart == vgSlut - 1) {
			...
			if (input.charAt(vgStart) == 'y')
				return input + " " + input.substring(0, vgStart) + 'ø'
