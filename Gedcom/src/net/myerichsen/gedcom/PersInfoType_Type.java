
/* First created by JCasGen Mon Oct 10 13:28:09 CEST 2011 */
package net.myerichsen.gedcom;

import org.apache.uima.jcas.JCas;
import org.apache.uima.jcas.JCasRegistry;
import org.apache.uima.cas.impl.CASImpl;
import org.apache.uima.cas.impl.FSGenerator;
import org.apache.uima.cas.FeatureStructure;
import org.apache.uima.cas.impl.TypeImpl;
import org.apache.uima.cas.Type;
import org.apache.uima.cas.impl.FeatureImpl;
import org.apache.uima.cas.Feature;
import org.apache.uima.jcas.tcas.Annotation_Type;

/** 
 * Updated by JCasGen Wed Apr 10 09:47:37 CEST 2013
 * @generated */
public class PersInfoType_Type extends Annotation_Type {
  /** @generated */
  private final FSGenerator fsGenerator = 
    new FSGenerator() {
      public FeatureStructure createFS(int addr, CASImpl cas) {
  			 if (PersInfoType_Type.this.useExistingInstance) {
  			   // Return eq fs instance if already created
  		     FeatureStructure fs = PersInfoType_Type.this.jcas.getJfsFromCaddr(addr);
  		     if (null == fs) {
  		       fs = new PersInfoType(addr, PersInfoType_Type.this);
  			   PersInfoType_Type.this.jcas.putJfsFromCaddr(addr, fs);
  			   return fs;
  		     }
  		     return fs;
        } else return new PersInfoType(addr, PersInfoType_Type.this);
  	  }
    };
  /** @generated */
  public final static int typeIndexID = PersInfoType.typeIndexID;
  /** @generated 
     @modifiable */
  public final static boolean featOkTst = JCasRegistry.getFeatOkTst("net.myerichsen.gedcom.PersInfoType");
  /** @generated */
  final Feature casFeat_information;
 
  /** @generated */
  final int     casFeatCode_information;
  /** @generated */
  final Feature casFeat_date;
  /** @generated */
  final int     casFeatCode_date;
  /** @generated */
  final Feature casFeat_place;
    
  
 
  /** @generated */
  final int     casFeatCode_place;
  /** @generated */
  final Feature casFeat_pIType;
  /** @generated */
  final int     casFeatCode_pIType;
  /** initialize variables to correspond with Cas Type and Features
	* @generated */
  public PersInfoType_Type(JCas jcas, Type casType) {
    super(jcas, casType);
    casImpl.getFSClassRegistry().addGeneratorForType((TypeImpl)this.casType, getFSGenerator());

 
    casFeat_information = jcas.getRequiredFeatureDE(casType, "information", "uima.cas.String", featOkTst);
    casFeatCode_information  = (null == casFeat_information) ? JCas.INVALID_FEATURE_CODE : ((FeatureImpl)casFeat_information).getCode();

 
    casFeat_date = jcas.getRequiredFeatureDE(casType, "date", "net.myerichsen.gedcom.DateType", featOkTst);
    casFeatCode_date  = (null == casFeat_date) ? JCas.INVALID_FEATURE_CODE : ((FeatureImpl)casFeat_date).getCode();

 
    casFeat_place = jcas.getRequiredFeatureDE(casType, "place", "net.myerichsen.gedcom.PlaceType", featOkTst);
    casFeatCode_place  = (null == casFeat_place) ? JCas.INVALID_FEATURE_CODE : ((FeatureImpl)casFeat_place).getCode();

 
    casFeat_pIType = jcas.getRequiredFeatureDE(casType, "pIType", "net.myerichsen.gedcom.PITypeType", featOkTst);
    casFeatCode_pIType  = (null == casFeat_pIType) ? JCas.INVALID_FEATURE_CODE : ((FeatureImpl)casFeat_pIType).getCode();

  }
  /** @generated */ 
  public int getDate(int addr) {
        if (featOkTst && casFeat_date == null)
      jcas.throwFeatMissing("date", "net.myerichsen.gedcom.PersInfoType");
    return ll_cas.ll_getRefValue(addr, casFeatCode_date);
  }
  /** @generated */
  @Override
protected FSGenerator getFSGenerator() {return fsGenerator;}
  /** @generated */ 
  public String getInformation(int addr) {
        if (featOkTst && casFeat_information == null)
      jcas.throwFeatMissing("information", "net.myerichsen.gedcom.PersInfoType");
    return ll_cas.ll_getStringValue(addr, casFeatCode_information);
  }
  /** @generated */ 
  public String getPIType(int addr) {
        if (featOkTst && casFeat_pIType == null)
      jcas.throwFeatMissing("pIType", "net.myerichsen.gedcom.PersInfoType");
    return ll_cas.ll_getStringValue(addr, casFeatCode_pIType);
  }
  /** @generated */ 
  public int getPlace(int addr) {
        if (featOkTst && casFeat_place == null)
      jcas.throwFeatMissing("place", "net.myerichsen.gedcom.PersInfoType");
    return ll_cas.ll_getRefValue(addr, casFeatCode_place);
  }
  /** @generated */    
  public void setDate(int addr, int v) {
        if (featOkTst && casFeat_date == null)
      jcas.throwFeatMissing("date", "net.myerichsen.gedcom.PersInfoType");
    ll_cas.ll_setRefValue(addr, casFeatCode_date, v);}
    
  
 
  /** @generated */    
  public void setInformation(int addr, String v) {
        if (featOkTst && casFeat_information == null)
      jcas.throwFeatMissing("information", "net.myerichsen.gedcom.PersInfoType");
    ll_cas.ll_setStringValue(addr, casFeatCode_information, v);}
    
  
 
  /** @generated */    
  public void setPIType(int addr, String v) {
        if (featOkTst && casFeat_pIType == null)
      jcas.throwFeatMissing("pIType", "net.myerichsen.gedcom.PersInfoType");
    ll_cas.ll_setStringValue(addr, casFeatCode_pIType, v);}
    
  



  /** @generated */    
  public void setPlace(int addr, int v) {
        if (featOkTst && casFeat_place == null)
      jcas.throwFeatMissing("place", "net.myerichsen.gedcom.PersInfoType");
    ll_cas.ll_setRefValue(addr, casFeatCode_place, v);}
    
  
 
}



    