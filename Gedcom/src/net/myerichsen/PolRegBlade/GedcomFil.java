...
public class GedcomFil {
	 ...
	 */
	public String hentGedcomTilVindue(final File gedcomFile) throws PolRegException {
		final StringBuffer sb = new StringBuffer();
		try {
			String line = null;
			final BufferedReader input = new BufferedReader(
					new FileReader(gedcomFile));
	 ...
	 * @throws PolRegException
	 */
	public void parse(final Koen koen) throws PolRegException {
			...
			while ((line = reader.readLine()) != null) {
				if (line.length() > 0) {
					...
					if (line.equals("Hovedperson")) {
						...
						hovedPerson.setFoedeDato(subLine[1]);
					...
					// TODO Håndtering af 1 NAME Anna Elisabeth Sofie Andresen
					// (født /Bundt)/
					// og Clara Caroline Nielsen (født Valentin) , 6-1-1858 , Rudkjøbing Ret
					if (line.equals("Ægtefælle")) {
						...
						denneAegteFaelle.setFoedeDato(subLine[1]);
						...
						if (subLine.length > 3) {
							if (subLine[3].startsWith("død ")) {
								...
								denneAegteFaelle.setDoedsDato(dato);
							}
						}
					...
					if (line.equals("Barn")) {
						...
						detteBarn.setKoen(Koen.BARN);
						detteBarn.setFoedeDato(subLine[1]);
						// Fjernet 25.04.2011
...
//								"FAMC @F" + foersteFamilieIdPaaBlad + "@");
	 ...
	 * @throws PolRegException
	 */
	public void udskriv(final File gedcomFile) throws PolRegException {
