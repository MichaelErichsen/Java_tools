...
public class ProbateAnnotator extends JCasAnnotator_ImplBase {
	...
	private EventRecType createEvent(JCas aJCas, int idCounter, int minBegin,
		...
		while (dateIter.hasNext()) {
			...
			d = date.getCoveredText().replaceAll("--", "01");
			
			try {
				formatter.applyPattern("dd MM yyyy");
				aDate = formatter.parse(d);
				formatter.applyPattern("yyyy-MM-dd");
				sb = new StringBuffer(formatter.format(aDate));
				d = sb.toString();
				event.setDate(d);
				event.setToDate(d);
			} catch (ParseException e) {
				System.out.println(e.getMessage());
				event.setDate("0001-01-01");
		...
		citation.setCaption("Asger Bruuns skifteuddrag");
