...
public class ProbateAnnotator extends JCasAnnotator_ImplBase {
	...
	private String cleanupDate(String ds) {
		String patternString1 = "\\d{4}\\s\\d{1,2}/\\d{1,2}";
		String patternString2 = "\\d{4}\\.\\s\\d{1,2}.\\s(jan|feb|marts|april|maj|juni|juli|aug|sep|okt|nov|dec).";
		String out = ds.trim();
		String s = "0000-01-01";
		String[] dateA;
		...
		pattern = Pattern.compile(patternString1, Pattern.CASE_INSENSITIVE);
		...
		if (matcher.matches()) {
			...
			pattern = Pattern.compile(patternString2, Pattern.CASE_INSENSITIVE);
			matcher = pattern.matcher(out);
			...
			if (matcher.matches()) {
				...
				logger.log(level, s);
				dateA = s.split("\\s");
				s = dateA[0] + " " + dateA[2].substring(0, 3) + " " + dateA[1];
	...
	private EventRecType createEvent(JCas aJCas, int idCounter, int start,
		...
		citation.setCaption("Bornholm Slægtssøgning");
